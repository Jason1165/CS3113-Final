...
public:
    // ----- STATIC VARIABLES ----- //
    static constexpr int SECONDS_PER_FRAME = 8; // FIGURE OUT HOW TO MAKE THIS INTO A MEMBER

    // ----- METHODS ----- //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, std::vector<std::vector<int>>, float animation_time,
    ...
    void face_idle_left() { m_animation_indices = m_walking[IDLE_LEFT]; }
    void face_idle_right() { m_animation_indices = m_walking[IDLE_RIGHT]; }
    //void face_hit() { m_animation_indices = m_walking[HIT]; }
